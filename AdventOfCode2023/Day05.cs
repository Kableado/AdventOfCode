...
public class Day05 : IDay
{
    public string ResolvePart1(string[] inputs)
    {
        ...
        long minLocation = almanac?.Seeds.Select(seed => almanac.ApplyMapping(seed)).Min() ?? 0;
    ...
    public readonly struct AlmanacRangeMapping
    {
    ...
    public class AlmanacMapping
    {
        ...
        public static AlmanacMapping? ParseNext(LinesReader reader)
        {
            ...
            do
            {
                strMapping = reader.GetLine();
            } while (string.IsNullOrWhiteSpace(strMapping) && strMapping != null);
            if (strMapping == null) { return null; }

            if (strMapping.EndsWith(mapKeyword) == false) { return null; }
            AlmanacMapping mapping = new() {
                Name = strMapping.Substring(0, strMapping.Length - mapKeyword.Length),
            };

            do
            {
                strMapping = reader.GetLine();
                if (string.IsNullOrEmpty(strMapping)) { break; }

                string[] mappingParts = strMapping.Split(' ');
        ...
        {
            int i = 0;
            while (i < RangeMappings.Count)
            {
                long? valueMapped = RangeMappings[i].Apply(value);
                if (valueMapped != null) { return valueMapped.Value; }
                i++;
            }
            return value;
        }
    ...
    {
        private Almanac(List<long> seeds)
        {
            Seeds = seeds;
        }
        ...
        {
            const string seedKeyword = "seeds: ";
            string? lineSeed = reader.GetLine();
            if (lineSeed == null) { return null; }
            if (lineSeed.StartsWith(seedKeyword) == false) { return null; }

            List<long> seeds = lineSeed.Substring(seedKeyword.Length).Split(' ').Select(s => Convert.ToInt64(s)).ToList();
            Almanac almanac = new(seeds);

            while (AlmanacMapping.ParseNext(reader) is { } mapping)
            {
                almanac.Mappings.Add(mapping);
            }
