...
public class Day04 : IDay
{
    ...
    {
        List<Card> cards = inputs.Select(input => new Card(input)).ToList();
        for (int i = 0; i < cards.Count; i++)
        {
            int matches = cards[i].CountMatches();
            for (int j = i + 1; j < (i + 1 + matches) && j < cards.Count; j++)
            {
    ...
    private class Card
    {
        ...
        private readonly List<int> _myNumbers;

        public int Count { get; set; } = 1;

        public Card(string strCard)
        {
            //const string cardKeyword = "Card ";
            string[] parts = strCard.Split(':');
            //_cardNumber = Convert.ToInt32(parts[0].Substring(cardKeyword.Length));
            string[] strColumns = parts[1].Split('|');
